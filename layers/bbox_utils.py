...
def jaccard(box_a, box_b):
    #计算ground truth boxes and default boxes重叠的地方
...
#将anchor(prior boxes)和truths进行匹配
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):   #打标签
    """Match each prior box with the ground truth box of the highest jaccard
    overlap, encode the bounding boxes, then return the matched indices
    corresponding to both confidence and location preds.
    #将每个先验框与ground truth框进行匹配，计算其中两个框的重叠部分得到最佳匹配的框
    #然后返回最佳匹配的框的索引以及对应的confidence和位置
    Args:
