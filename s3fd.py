...
class S3FD(nn.Module):
    ...
    #网络由基本的VGG网络和一些其他的卷积层组成，每个多盒层分支成
        1) conv2d for class conf scores
        2) conv2d for localization predictions
        3) associated priorbox layer to produce default bounding
           boxes specific to the layer's feature map size.
    #与priorbox layer结合来生成对应层特征图大小的默认框
...
vgg_cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M',
           512, 512, 512, 'M']
#一些参数的设置,用以调用vgg()函数时方便构建网络
extras_cfg = [256, 'S', 512, 128, 'S', 256]

#构建vgg的网络
def vgg(cfg, i, batch_norm=False):
    layers = []
    in_channels = i  #channels=3表示输入的是rgb图
    for v in cfg:
        if v == 'M':
...
#这些为除了vgg网络外额外增加的层,在vgg网络的后面
def add_extras(cfg, i, batch_norm=False):
    # Extra layers added to VGG for feature scaling
    layers = []
    in_channels = i     #channels=1024(vgg的输出)
    flag = False
