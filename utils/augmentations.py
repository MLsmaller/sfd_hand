...
def distort_image(img):
    #uniform:一致的,均匀的  np.random.uniform(low,high,size)从一个均匀分布[low,high)中随机采样size个数
    prob = np.random.uniform(0, 1)
    # Apply different distort order
    if prob > 0.5:
        img = random_brightness(img)    #光照.对比度..明亮等
...
def expand_image(img, bbox_labels, img_width, img_height):
    ...
    if prob < cfg.expand_prob:
        if cfg.expand_max_ratio - 1 >= 0.01:
            ...
            h_off = math.floor(np.random.uniform(0, height - img_height)) #math.floor()向下取整
            w_off = math.floor(np.random.uniform(0, width - img_width))
            #h_off和w_off是随机取的一个值,这样每张训练图片扩展后再新图片中的位置都不一样,增加了样本的鲁棒性
            expand_bbox = bbox(-w_off / img_width, -h_off / img_height,  #bbox为一个类
                               (width - w_off) / img_width,
                               (height - h_off) / img_height)
            #expand_bbox为图片变大后原来的图片在新图片中坐标的最大和最小值(坐标系还是以原来图片的左上角为原点)
            #因为后面expand_img.paste进行粘贴图片时左上角坐标为(int(w_off),int(h_off))
            expand_img = np.ones((height, width, 3))
            #np.squeeze()函数去掉维度为1的维度,在这里expand_img为b,g,r分量分别为img_mean中的值的图片
            expand_img = np.uint8(expand_img * np.squeeze(cfg.img_mean))
            expand_img = Image.fromarray(expand_img) #如果需要保存的图片变量是数组array形式,需要先转换为Image格式,然后再保存
            expand_img.paste(img, (int(w_off), int(h_off)))  #将图片img粘贴到图片expand_img中,(int(w_off),int(h_off))为被粘贴的图片在新图片中左上角的位置
...
def anchor_crop_image_sampling(img,
    ...
    #np.random.randint(n)产生一个小于n的随机数
...
def preprocess(img, bbox_labels, mode, image_path):
    ...
    if mode == 'train':
        if cfg.apply_distort:  #一些数据处理(光照等)
    ...
    img = np.array(img)
    #::双冒号其实相当于start:end:step,出现::其实是省略了end,当step为负数时,默认start为-1,即从最后一个元素开始取
    #range(5)[::2]=[0,2,4] ，::2意思是每两个取一个数,相当于步长step
    #s=range(20)  s[2::3]=[2,5,8...]
    #::-1步长为-1,倒过来看最后一个位置为-1,::-1可以看做是倒序
    if mode == 'train':
    ...
    img -= cfg.img_mean
    #to_chw_bgr()函数这里将图片的通道channels换了位置,本来是H,W,C,变成C,H,W
    #下面代码相当于说将图片的通道换了位置,bgr_rgb,而height和width不变
